...
async def chat_step(
    ...
    if SCREENSHOT_SAVE_DIR:
        try:
            ...
            print(f"[Server] Saved screenshot to: {filepath}")
        except Exception as e:
            print(f"[Server] Failed to save screenshot: {e}")
...
if __name__ == "__main__":
    ...
    if args.save_dir:
        SCREENSHOT_SAVE_DIR = args.save_dir
        if not os.path.exists(SCREENSHOT_SAVE_DIR):
            os.makedirs(SCREENSHOT_SAVE_DIR)
        print(f"[Server] Saving screenshots to: {SCREENSHOT_SAVE_DIR}")
